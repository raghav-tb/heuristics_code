...
def bhav_copy_df_format():
	...
	df = df[df['instrument_type_bhav'].isin(['FUTIDX','FUTSTK'])]
	scheme_rename_mapper = {
		"FUTIDX": "FUTURES",
		'FUTSTK': "FUTURES"
	}
...
def calculate_unique_id(df):
	df['unique_id'] = df['symbol_code'] + pd.to_datetime(df.symbol_name.str[-10:], format="%d/%m/%Y").dt.strftime('%y%b').str.upper() + 'FUT'
	# print(str(df['symbol_code'] + pd.to_datetime(df.expiry_date, format="%d/%m/%Y").dt.strftime('%y%b') + 'FUT').upper())
	return df
...
# Conflicted dataframe merge with bhav_copy and calculate unique_id:
	if len(conflicted_df.index) != 0:
		...
		conflicted_df = conflicted_df[ (conflicted_df['expiry_date'] == conflicted_df['symbol_name'].str[-10:]) ]
		conflicted_df = conflicted_df.drop_duplicates(keep=False)
